...
bool operator<(const TokenIndex &a, const TokenIndex &b) { return a.str < b.str; }
...
class Vocabulary {
   public:
    ...
    auto getPiece(const int token) -> char * { return m_vocab[token].data(); }

    auto query(const std::string &query_str) -> int { return str_lookup(query_str, m_sorted_vocab.data(), vocab_size); }

    bool isInitialized() { return !m_sorted_vocab.empty(); }
    ...
    void addVocab(const int index, const word_t &word) { m_vocab.at(index) = word; }
    ...
    auto size() const -> const size_t { return m_vocab.size(); }
...
class Tokenizer : public TokenizerBase {
   public:
    ...
    Tokenizer(const std::string &tokenizer_path, int vocab_size_) : TokenizerBase() { loadTokenizer(tokenizer_path, vocab_size_); }
