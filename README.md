...
1. Запустить проект на x86 машине, используя [инструкции](#feel-the-magic) ниже
2. Сделать копию репозитория (fork)
3. Написать несколько тестов в файле [test.c](./test.c) для функции `generate` из файла [run.c](run.c). Данные получить несколькими запусками программы на разных текстах (`-i`) и параметрах алгоритма (`-t`, `-p`). От каждого участника проекта ожидается открытие PR со своим вариантом теста.
4. Запустить проект на RISC-V плате, убедиться что все тесты проходят.
