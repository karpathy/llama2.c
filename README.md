...
2. Запустить проект на x86 машине, используя [инструкции](#feel-the-magic) ниже (используйте ветку `master`)
3. Запустить проект на RISC-V плате, убедиться что все тесты проходят (ветка `feature/rvv`). Используйте компилятор https://drive.google.com/uc?id=1U3JaUqWyRYvlhfA3lVEfwu56em-Gwonl (перед компиляцией вызвать один раз команду `export PATH=/path/to/xuantie-gcc/bin/:$PATH`, указав путь до распакованного компилятора).
4. Написать несколько тестов в файле [test.c](./test.c) для функции `generate` из файла [run.c](run.c). Данные получить несколькими запусками программы на разных текстах (`-i`) и параметрах алгоритма (`-t`, `-p`). От каждого участника проекта ожидается открытие PR со своим вариантом теста. На тестовой плате модели для запуска лежат в папке `/tmp`.
5. Провести анализ производительности. Убедиться в устойчивости полученных значений между запусками.
    - Общая скорость генерации tokens per second (tok/s) - подсчитывается самой программой по завершению генерации
    - Hotspot analysis - измерить производительность каждой отдельной функции в [run.c](run.c), собрать статистику и выбрать наиболее трудоемкую для оптимизации
6. Сделать раунд оптимизации конкретной функции и вернуться на шаг 4.

Результатом каждой итерации является открытый Pull Request с таблицей вида

| Было (tok/s) | Стало (tok/s) | Ускорение |
|--------------|---------------|-----------|
|           14 |            17 |     x1.21 |

7. О каждого участника команды требуется создать ветку с названием вида `имя/feature/rvv` (например `dkurtaev/feature/rvv`). В неё же последовательно открывать PR с предложенными оптимизациями. По завершению работы над проектом, собрать результаты оптимизации от всех участников в презентацию (все замеры должны быть произведены при одних и тех же условиях). По взаимной договоренности, работу по пунктам 4-6 можно разделить таким образом, что каждому из участников назначается отдельная функция.
8. (Опционально) Провести анализ работы каждого из участников и собрать все решения в один PR. Подсчитать общее ускорение относительно базовой версии библиотеки.


Далее оригинальный текст README.md файла проекта https://github.com/karpathy/llama2.c:
