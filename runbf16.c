...
void memory_map_weights(TransformerWeights *w, Config* p, float* ptr, int shared_weights) {
    ...
    ptr += n_layers * p->dim;
    w->wq = (bf16 *)ptr;
    ptr += (n_layers * p->dim * (p->n_heads * head_size)) / 2;
    w->wk = (bf16 *)ptr;
    ptr += (n_layers * p->dim * (p->n_kv_heads * head_size)) / 2;
    w->wv = (bf16 *)ptr;
    ptr += (n_layers * p->dim * (p->n_kv_heads * head_size)) / 2;
    w->wo = (bf16 *)ptr;
    ptr += (n_layers * (p->n_heads * head_size) * p->dim) / 2;
    w->rms_ffn_weight = ptr;
    ptr += n_layers * p->dim;
    w->w1 = (bf16 *)ptr;
    ptr += (n_layers * p->dim * p->hidden_dim) / 2;
    w->w2 = (bf16 *)ptr;
    ptr += (n_layers * p->hidden_dim * p->dim) / 2;
    w->w3 = (bf16 *)ptr;
    ptr += (n_layers * p->dim * p->hidden_dim) / 2;
    ...
    w->wcls = (bf16 *)(shared_weights ? w->token_embedding_table : ptr);
}
