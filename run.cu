...
half* load_weights(int elements, float** pptr, void *scratchGPU) {
    half* w = NULL;
    cudaMalloc((void**)&w, elements * sizeof(half));
    if (!w) {
        fprintf(stderr, "malloc failed!\n");
        exit(EXIT_FAILURE);
    }
    // copy and convert fp32->fp16
    cudaMemcpyAsync(scratchGPU, *pptr, elements * sizeof(float), cudaMemcpyHostToDevice);
    convert_fp32_to_fp16 <<< divUp(elements, 1024), 1024 >>> (w, (float*)scratchGPU, elements);
    *pptr += elements;
    return w;
}

void load_checkpoint_weights(TransformerWeights* w, Config* p, float* ptr, int shared_weights) {
    ...
    w->token_embedding_table = load_weights(p->vocab_size * p->dim, &ptr, scratchGPU);

    w->rms_att_weight = load_weights(p->n_layers * p->dim, &ptr, scratchGPU);

    w->wq = load_weights(p->n_layers * p->dim * (p->n_heads * head_size), &ptr, scratchGPU);
    w->wk = load_weights(p->n_layers * p->dim * (p->n_kv_heads * head_size), &ptr, scratchGPU);
    w->wv = load_weights(p->n_layers * p->dim * (p->n_kv_heads * head_size), &ptr, scratchGPU);
    w->wo = load_weights(p->n_layers * (p->n_heads * head_size) * p->dim, &ptr, scratchGPU);

    w->rms_ffn_weight = load_weights(p->n_layers * p->dim, &ptr, scratchGPU);

    w->w1 = load_weights(p->n_layers * p->dim * p->hidden_dim, &ptr, scratchGPU);
    w->w2 = load_weights(p->n_layers * p->hidden_dim * p->dim, &ptr, scratchGPU);
    w->w3 = load_weights(p->n_layers * p->dim * p->hidden_dim, &ptr, scratchGPU);

    w->rms_final_weight = load_weights(p->dim, &ptr, scratchGPU);
    ...
    if (shared_weights)
        ...
        w->wcls = load_weights(p->vocab_size * p->dim, &ptr, scratchGPU);

    cudaFree(scratchGPU);
}

void read_checkpoint(char* checkpoint, Config* config, TransformerWeights* weights,
                     int* fd, float** data, ssize_t* file_size, int *shared_weights) {
